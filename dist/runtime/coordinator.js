...
export class ArrangementCoordinator {
    ...
    router;
    factory;
    constructor(manifest, credentials, archiveDirectory = './memory', channelDirectory = './transfer') {
    ...
    async inspectStatus() {
        const personaStatistics = await Promise.all(this.manifest.objective.personas.map(async (blueprint) => ({
