...
export class ArchiveManager {
    rootDirectory;
    constructor(rootDirectory = './memory') {
        this.rootDirectory = rootDirectory;
    }
    async preparePersonaArchive(personaId) {
...
`;
    ...
    async retrieveChronicles(personaId, maxRecords) {
        ...
        try {
            ...
            return Promise.all(limitedFiles.map(async (filename) => {
    ...
    async gatherStatistics(personaId) {
        const archivePath = pathJoin(this.rootDirectory, personaId);
        try {
            const entries = await fsPromises.readdir(archivePath);
