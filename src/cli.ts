...
cli.command('run').action(async () => {
  const flowConfig = {
    roles: [
      { name: 'splitter', type: 'splitter' as const, geminiSettings: { temp: 0.3, tokens: 4096 } },
      { name: 'worker', type: 'worker' as const, geminiSettings: { temp: 0.7, tokens: 4096 }, parallel: 2 },
      { name: 'validator', type: 'validator' as const, geminiSettings: { temp: 0.1, tokens: 2048 } },
      { name: 'merger', type: 'merger' as const, geminiSettings: { temp: 0.3, tokens: 4096 } }
    ],
    connections: [
